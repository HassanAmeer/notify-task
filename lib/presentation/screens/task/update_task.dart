...
class _UpdateTaskPageState extends State<UpdateTaskPage> {
  ...
  void initState() {
    super.initState();
    _taskName.text = widget.taskData.taskName;
    _category = widget.taskData.catg; // Default category
    _dateTime = DateTime.parse(
        widget.taskData.dateTime); // Changed to DateTime for date and time
    _status = widget.taskData.status == "pending" ? false : true;
    _description.text = widget.taskData.taskDesc;
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Update Task'), actions: [
        IconButton(
            onPressed: () {
              showCupertinoDialog(
                  context: context,
                  builder: (BuildContext context) {
                    ...
                    return CupertinoAlertDialog(
                      title: const Text('Share By User Email'),
                      content: CupertinoTextField(controller: emailController),
                      actions: [
                        CupertinoButton(
                            onPressed: () {
                        ...
                        CupertinoButton(
                            onPressed: () {
                              context.read<AuthBloc>().add(
                                  TaskShareByEmailEvent(
                                      context: context,
                                      email: emailController.text,
                                      userTask: widget.taskData));
                              Navigator.pop(context);
                            },
        ...
        IconButton(
            ...
            onPressed: () {
              deleteAccountAlert(context, title: "Want to Delete ?", onTap: () {
                List<UserTask> taskList = [];
                taskList = widget.TaskList;
                taskList.removeAt(widget.taskIndex);
                context.read<AuthBloc>().add(
                    TaskUpdateEvent(context: context, userTaskList: taskList));
                Navigator.pop(context);
              });
            })
      ]),
      ...
      bottomNavigationBar: Padding(
        ...
        child: ElevatedButton(
          ...
          onPressed: () async {
            List<UserTask> taskList = [];
            taskList = widget.TaskList;
            taskList[widget.taskIndex] = UserTask(
